...
- (BOOL)isEqualToSection:(WMFExploreSection *)rhs {
    return self.type == rhs.type && WMF_RHS_PROP_EQUAL(dateCreated, isEqualToDate:) && WMF_RHS_PROP_EQUAL(site, isEqualToSite:) && WMF_RHS_PROP_EQUAL(title, isEqualToTitle:) && WMF_RHS_PROP_EQUAL(mostReadFetchDate, isEqualToDate:) && WMF_RHS_PROP_EQUAL(location, wmf_isEqual:) && WMF_RHS_PROP_EQUAL(placemark, wmf_isEqual:);
}
